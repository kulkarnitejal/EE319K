...
high 	LDR R2,[R1]
		...
		CMP R2,#0X00000010						;check if busy
		BEQ high								;loop if busy
		LDR R1,=DC								
		LDR R2,=DC_COMMAND
		STR R2,[R1]								;store DC_command in to DC (set to zero)
		LDR R1, =SSI0_DR_R
		STRB R0,[R1]							;data register gets command
		LDR R1, =SSI0_SR_R
high2 	LDR R2,[R1]								
		AND R2,R2,#0X00000010					;if high, wait for low
		CMP R2,#0X00000010
		BEQ high2
...
;4) Write the 8-bit data to SSI0_DR_R
    LDR R1, =SSI0_SR_R
low 	LDR R2,[R1]								
		AND R2,R2,#0X00000002					;check data bit 1 for 0
		CMP R2,#0
		BEQ low									;loop if 0
		LDR R1,=DC								
		LDR R2,=DC_DATA	
		STR R2,[R1]								;store dc_data in dc
		LDR R1, =SSI0_DR_R						
		STRB R0,[R1]							;data register gets data
